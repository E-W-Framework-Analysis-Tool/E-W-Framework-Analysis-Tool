using EdFi.OdsApi.Sdk.Models.Ed_Fi;
using EwFrameworkAnalysis.Common.Models.Project;
using EwFrameworkAnalysis.Common.Services;

namespace EwFrameworkAnalysis.Common.Assessors.EdFi;

public class TeacherChildInteractionMeasureK12EdFiAssessor : IEdFiAssessor
{
    private const string TeacherCategoryDescriptor =
        "uri://ed-fi.org/SurveyCategoryDescriptor#Teacher";

    private static readonly string[] _surveyKeywords =
    [
        "Classroom Assessment Scoring System",
        "teacher-child interaction",
        "teacher child interaction",
        "instructional support",
        "emotional support",
        "classroom organization",
        "TSRS",
        "Teacher-Student Relationship Scale",
        "interaction quality"
    ];

    public string DataElementName => "Teacher-child interaction measure (K-12)";

    public string AssessmentDescription =>
        "Identifies teacher-category surveys linked to K-12 teacher-child interaction measures " +
        "(e.g., CLASS — Classroom Assessment Scoring System, Teacher-Student Relationship Scale) " +
        "by filtering on the Teacher survey category descriptor and matching well-known instrument " +
        "names and interaction quality keywords in the Ed-Fi surveys catalog.";

    public async Task<DataElementAssessment> AssessAsync(
        HttpClient httpClient,
        DataSource dataSource,
        AssessorContext context)
    {
        context.ReportProgress(0, "Searching teacher surveys for K-12 teacher-child interaction measures...");

        var totalAdministered = 0;
        var surveyDistribution = new Dictionary<string, int>();

        var queryParams = new Dictionary<string, string>
        {
            ["surveyCategoryDescriptor"] = TeacherCategoryDescriptor
        };

        await EdFiApiPatterns.PageAndProcessAsync<EdFiSurvey>(
            httpClient,
            "ed-fi/surveys",
            survey =>
            {
                if (!IsTeacherChildInteractionSurvey(survey))
                    return;

                var administered = survey.NumberAdministered ?? 0;
                totalAdministered += administered;
                surveyDistribution[survey.SurveyIdentifier] = administered;
            },
            context,
            queryParams);

        context.Log($"Found {surveyDistribution.Count} K-12 teacher-child interaction measure(s) " +
                     $"with {totalAdministered:N0} total administered");
        context.ReportProgress(100, "Complete");

        if (surveyDistribution.Count == 0)
        {
            return new DataElementAssessment
            {
                DataElementName = DataElementName,
                Characteristics = [new RecordCount(0)],
                Remarks = AssessmentDescription +
                    " No surveys matching recognized K-12 teacher-child interaction instruments " +
                    "were found in the survey catalog."
            };
        }

        return new DataElementAssessment
        {
            DataElementName = DataElementName,
            Characteristics =
            [
                new RecordCount(totalAdministered),
                new Distribution(surveyDistribution, "Survey Instrument")
            ],
            Remarks = AssessmentDescription
        };
    }

    private static bool IsTeacherChildInteractionSurvey(EdFiSurvey survey)
    {
        var title = survey.SurveyTitle ?? string.Empty;
        var identifier = survey.SurveyIdentifier ?? string.Empty;

        foreach (var keyword in _surveyKeywords)
        {
            if (title.Contains(keyword, StringComparison.OrdinalIgnoreCase) ||
                identifier.Contains(keyword, StringComparison.OrdinalIgnoreCase))
                return true;
        }

        return false;
    }
}
