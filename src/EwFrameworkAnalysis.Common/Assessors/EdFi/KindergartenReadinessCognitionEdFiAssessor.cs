...
public class KindergartenReadinessCognitionEdFiAssessor : IEdFiAssessor
{
    ...
    public string AssessmentDescription =>
        ...
        "(e.g., KRA — Kindergarten Readiness Assessment, DIAL — Developmental Indicators for " +
        "the Assessment of Learning, Bracken Basic Concept Scale, Boehm Test of Basic Concepts) " +
        "by matching well-known instrument names and readiness/cognition keywords in the Ed-Fi " +
        "assessments catalog.";
    ...
    {
        context.ReportProgress(0, "Searching assessment catalog for kindergarten readiness cognition assessments...");
        ...
        await EdFiApiPatterns.PageAndProcessAsync<EdFiAssessment>(
            ...
            assessment =>
            {
                if (IsKindergartenReadinessCognitionAssessment(assessment))
                    matchingAssessments.Add((assessment.AssessmentIdentifier, assessment.Namespace));
            },
            context);

        context.Log($"Found {matchingAssessments.Count} kindergarten readiness cognition assessment(s) in catalog");

        if (matchingAssessments.Count == 0)
        {
            context.ReportProgress(100, "Complete — no kindergarten readiness cognition assessments found");
            return new DataElementAssessment
            {
                DataElementName = DataElementName,
                Characteristics = [new RecordCount(0)],
                Remarks = AssessmentDescription +
                    " No assessments matching recognized kindergarten readiness cognitive " +
                    "instruments were found in the assessment catalog."
            };
        }

        context.ReportProgress(50, "Counting student assessment records for readiness cognition instruments...");
        ...
        context.Log($"Found {totalCount:N0} kindergarten readiness cognition assessment records");
    ...
    private static bool IsKindergartenReadinessCognitionAssessment(EdFiAssessment assessment)
    {
