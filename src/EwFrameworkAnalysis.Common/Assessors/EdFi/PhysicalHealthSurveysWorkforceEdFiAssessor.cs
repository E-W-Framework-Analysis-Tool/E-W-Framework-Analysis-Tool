...
public class PhysicalHealthSurveysWorkforceEdFiAssessor : IEdFiAssessor
{
    private readonly EdFiStudentAssessmentProvider _assessmentProvider;

    public PhysicalHealthSurveysWorkforceEdFiAssessor(EdFiStudentAssessmentProvider assessmentProvider)
    {
        _assessmentProvider = assessmentProvider;
    }

    public string DataElementName => "Physical health surveys (Workforce)";

    public string AssessmentDescription =>
        "Analyzes studentAssessments as a baseline proxy for workforce physical health surveys. " +
        "Ed-Fi has no standard descriptor for physical health survey categories.";
