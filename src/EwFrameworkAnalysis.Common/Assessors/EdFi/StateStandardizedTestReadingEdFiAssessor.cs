...
// Grade-agnostic state standardized reading/ELA assessment (all grades). Grade-level breakdown is
// reported as a "Grade Level Assessed" distribution derived from each assessment's AssessedGradeLevels.
public class StateStandardizedTestReadingEdFiAssessor : IEdFiAssessor
{
    ...
    public string DataElementName => "State standardized test (Reading proficiency)";

    public string AssessmentDescription =>
        "Count of students with state standardized test results in English Language Arts or Reading across " +
        "all grade levels, broken down by the grade level(s) each assessment is administered for.";
    ...
    {
        context.ReportProgress(0, "Finding state standardized ELA/Reading assessments...");
        ...
        context.Log($"Found {assessmentGradeLevels.Count} state standardized ELA/Reading assessments");
        ...
        context.Log($"Found {totalCount:N0} state standardized ELA/Reading assessment records");
        context.ReportProgress(100, "Complete");
