...
/// <summary>
/// Profiles reading state standardized test records for grade 3 from
/// RDS.FactK12StudentAssessments joined to RDS.DimAssessments and
/// RDS.FactK12StudentEnrollments (for grade level). Filters to
/// AssessmentAcademicSubjectCode = '00560' (Reading) and GradeLevelCode = '03'.
...
/// </summary>
public class StateStandardizedTestReadingProficiencyCedsDWAssessor : ICedsDWAssessor
{
    public string DataElementName => "State standardized test (Reading proficiency)";

    public string Query => $@"
WITH ReadingSummativeBase AS (
    SELECT
        da.AssessmentTitle,
        CASE
    ...
    WHERE da.AssessmentTypeCode IN (
      ...
      AND da.AssessmentAcademicSubjectCode = '00560'
...
LEFT JOIN TitleCounts t ON 1 = 1";
    ...
    public string AssessmentDescription =>
        "Assesses reading state standardized test records for grade 3 from " +
        "RDS.FactK12StudentAssessments joined to RDS.DimAssessments and " +
        "RDS.FactK12StudentEnrollments. Filters to AssessmentAcademicSubjectCode = '00560' " +
