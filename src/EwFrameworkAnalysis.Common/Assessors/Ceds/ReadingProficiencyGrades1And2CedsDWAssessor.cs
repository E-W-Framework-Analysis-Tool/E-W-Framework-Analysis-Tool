...
/// AssessmentTypeCode = 'Benchmark' and AssessmentAcademicSubjectCode = '00560'
/// (Reading), scoped to GradeLevelCode IN ('01', '02').
...
/// </summary>
public class ReadingProficiencyGrades1And2CedsDWAssessor : ICedsDWAssessor
{
    public string DataElementName => "Reading proficiency (Grades 1 and 2)";

    public string Query => $@"
WITH ReadingBenchmarkBase AS (
    SELECT
        da.AssessmentTitle,
        CASE
...
LEFT JOIN TitleCounts t ON 1 = 1";
    ...
    public string AssessmentDescription =>
        "Assesses reading benchmark assessment records for grades 1 and 2 from " +
        "RDS.FactK12StudentAssessments joined to RDS.DimAssessments and " +
