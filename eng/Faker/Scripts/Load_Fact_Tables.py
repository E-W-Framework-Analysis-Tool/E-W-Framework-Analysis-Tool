    ...
    try :
        ...
        #     print("Base Table Data successfully loaded into SQL Server!")
            ...
            with engine.begin() as conn:  # auto-commit transaction
                    ...
                    dim_people = pd.read_sql_query("Select * from RDS.DimPeople_Current;", conn)
                    fact_enroll_history = pd.read_sql_query("Select p.K12StudentStudentIdentifierState, g.GradeLevelCode,y.SchoolYear from RDS.FactK12StudentEnrollments f \
                    join RDS.DimSchoolYears y on y.DimSchoolYearId = f.SchoolYearId join rds.DimPeople p on p.DimPersonId = f.K12StudentId \
                    join rds.DimGradeLevels g on f.ExitGradeLevelId = g.DimGradeLevelId \
                    where SchoolYear< YEAR(GETDATE())", conn)
            ...
            dim_people["SchoolYear"] = datetime.now().year
            dim_people["SchoolYearId"] = dim_people["SchoolYear"].map(school_year_map)
            ...
            dim_students = dim_people[dim_people['IsActiveK12Student']==1]

            for index, student in dim_students.iterrows():
                    # get grades from history and assign the next grade to latest record
                    grades = fact_enroll_history[fact_enroll_history['K12StudentStudentIdentifierState'] == student['K12StudentStudentIdentifierState']]['GradeLevelCode']
                    grade  = -1
                    if not grades.empty:
                        grade = grades.astype(int).max() + 1
                    else:
                        grade = random.choice([1,13])
                    ...
                    start_dt = datetime.strptime(f"{student['SchoolYear']}-07-01", "%Y-%m-%d").date()

                    # Randomly pick end date: either June 30 of the same year or None
                    end_str = random.choice([f"{student['SchoolYear']+1}-06-30", None])
                    ...
                    fact_enrollments.append({
                                ...
                                "K12Student_CurrentId":student["DimPersonId"],
                                ...
                                "CohortYearId": -1,
                                "CteStatusId": -1,
                                "EntryGradeLevelId": grade_map.get(grade) if grade is not None else -1,
                                "ExitGradeLevelId": grade_map.get(min(grade + 1, 12)  ) if grade else -1,
                                ...
                                "K12EnrollmentStatusId": random.randint(1,570374),
                                "K12DemographicId": K12DemographicId,
                                "IdeaStatusId": -1,
                                "HomelessnessStatusId": 1 if hom_start else 0,
                                "EconomicallyDisadvantagedStatusId": random.choice([1,14]) if econ_start else -1,
                    ...
                    fact_assessments.append({
                            ...
                            "CteStatusId": -1,
                            "GradeLevelWhenAssessedId": grade_map.get(grade),
                            "IdeaStatusId": -1,
                            "K12DemographicId": K12DemographicId,
                            "NOrDStatusId": random.choice([-1,1,2]),
                            "TitleIIIStatusId": random.choice([-1,1]),
                    ...
                    fact_discipline.append({
                            ...
                            "FosterCareStatusId": random.choice([-1,1,2]),
                            "GradeLevelId": grade_map.get(grade),
                            "HomelessnessStatusId": 1 if hom_start else 0,
                            "IdeaStatusId": -1,
                            "ImmigrantStatusId": random.choice([-1,1,2]),
                            "IncidentIdentifier": f"INC-{random.randint(1, 5075)}",
            ...
            dim_staff = dim_people[dim_people['IsActiveK12Staff']==1]

            ###Add FactK12StaffCounts table here based on dim_staff
